...
def create(graph_type):
    ...
    with session_scope() as session:
        ...
        group = DesignTableGroup.get(session, uid = design_table.group_uid)
        phase = SitePhase.get(session, uid = phase_uid)
        now = datetime.now()
        filename = ""

        if group.name =="電包設計規範":
            filename = f"{site.name}_{design_table.name}_第{phase.sort}期_{now.strftime('%Y%m%d')}"
        else:
            filename = f"{site.name}_{group.name}_第{phase.sort}期_{now.strftime('%Y%m%d')}"
