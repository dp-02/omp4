...
class SitePhaseModule(db.Model):
    ''' 案場期數模組 '''
    __tablename__ = 'site_phase_module'

    uid = Column(Integer, primary_key=True, autoincrement=True)
    phase_uid = Column(Integer)
    brand = Column(String(256))
    model = Column(String(256))
    wattage = Column(String(256))
    
    phase_uid = Column(Integer, ForeignKey('site_phase.uid', ondelete='CASCADE'), nullable=False)
    phase = relationship("SitePhase", back_populates="modules")
    ...
    def __repr__(self):
        return f'<site_phase_module {self.uid}>'